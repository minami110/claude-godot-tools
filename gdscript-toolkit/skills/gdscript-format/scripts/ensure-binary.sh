...
FORMATTER_PATH="$BIN_DIR/gdscript-formatter"
...
binary_present() {
  [ -x "$FORMATTER_PATH" ] || [ -x "${FORMATTER_PATH}.exe" ]
}

needs_install=0
if ! binary_present; then
  echo "gdscript-formatter not found. Installing v${PINNED_VERSION}..." >&2
  needs_install=1
elif [ "$INSTALLED_VERSION" != "$PINNED_VERSION" ]; then
  echo "gdscript-formatter version drift: installed=${INSTALLED_VERSION:-unknown}, pinned=${PINNED_VERSION}. Reinstalling..." >&2
  needs_install=1
fi
...
if [ "$needs_install" -eq 1 ]; then
  ...
  echo "gdscript-formatter v${PINNED_VERSION} ready." >&2
fi
